...
int chooseCudaDevice() {
    ...
    int deviceChoice;
    printf("Select the device number to use: ");
    if (scanf("%d", &deviceChoice) != 1 || deviceChoice < 0 || deviceChoice >= deviceCount) {
        fprintf(stderr, "Invalid device choice!\n");
        return -1;
    }
...
int main() {
    ...
    int selectedDevice = chooseCudaDevice();
    if (selectedDevice == -1) {
        return 1;
    }

    // Add vectors in parallel using the selected device.
